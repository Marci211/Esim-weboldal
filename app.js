// Constants
const API_URL = 'packages.json';
const HUF_EXCHANGE_RATE = 360;
const PROFIT_MARGIN = 1.6; // 60% profit margin
...
async function fetchPackages() {
    try {
        ...
        document.getElementById('loading-text').innerText = 'Hálózati hiba. Kérjük frissítse az oldalt.';
    }
}
...
async function autoSetLanguage() {
    try {
        ...
        if (country === 'HU') currentLang = 'hu';
        else if (['DE', 'AT', 'CH'].includes(country)) currentLang = 'de';
        else currentLang = 'en';
    } catch(e) {
        console.error('IP API failed, fallback to English', e);
        currentLang = 'en';
    }
    applyTranslations();
}
...
function applyTranslations() {
    ...
    if (guideContent) guideContent.innerHTML = t.guideContent;
}
...
function setupEventListeners() {
    ...
    document.getElementById('btn-checkout').addEventListener('click', async () => {
        if (cart.length === 0) return alert('A kosarad üres / Your cart is empty!');
        const emailInput = document.getElementById('checkout-email').value;
        const nameInput = document.getElementById('checkout-name').value;
        if (!emailInput || !nameInput) return alert('Kérjük, adja meg a nevét és e-mail címét! / Please provide your name and email!');
        ...
        try {
            const BACKEND_URL = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') ? 'http://127.0.0.1:5000' : 'https://your-production-backend-url.com';
            const response = await fetch(BACKEND_URL + '/checkout', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ cart: cart, email: emailInput, name: nameInput })
            });

            if (response.ok) {
                alert('Sikeres fizetés! A QR kódokat elküldtük e-mailben. / Successful checkout! QR codes sent via email.');
                cart = [];
                saveCart();
                updateCartCount();
                renderCart();
                ...
                alert('Hiba történt a rendelés során. / Error during checkout.');
            }
        } catch (error) {
            alert('Hálózati hiba történt a szerver elérésekor. / Network error to server.');
        } finally {
